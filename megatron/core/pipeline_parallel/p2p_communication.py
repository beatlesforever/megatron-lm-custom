...
def _batched_p2p_ops(*,
    ...
    ops = []
    args = get_args()
    file_name = args.save + '/log%d.txt' % torch.distributed.get_rank()
    if tensor_send_prev is not None:
        send_prev_op = torch.distributed.P2POp(
        ...
        #     log_file.write('        send_prev_op: %s\n' % str(send_prev_op))
        #     log_file.write('        dst: %d\n' % get_pipeline_model_parallel_prev_rank())
    if tensor_recv_prev is not None:
        ...
        ops.append(recv_prev_op)
        # with open(file_name, 'a') as log_file:
        #     log_file.write('        recv_prev_op: %s\n' % str(recv_prev_op))
        #     log_file.write('        src: %d\n' % get_pipeline_model_parallel_prev_rank())
    if tensor_send_next is not None:
        send_next_op = torch.distributed.P2POp(
...
def recv_forward(tensor_shape: Shape,
                 ...
                 from_prev: bool = True) -> torch.Tensor:
    ...
    from_prev: in wpp, some micro batch may recv forward from next rank in pipeline
    """
    ...
    if core.parallel_state.is_pipeline_first_stage():
        ...
        if from_prev:
            ...
            # wppline: recv forward from next rank
...
def recv_backward(tensor_shape: Shape,
                  ...
                  batch_p2p_comm: bool = True,
                  timers: Callable = None,
                  from_next: bool = True) -> torch.Tensor:
    """Receive tensor from next rank in pipeline (backward receive).

    See _communicate for argument details.
    
    from_next: in wpp, some micro batch may recv backward from prev rank
    """
    if core.parallel_state.is_pipeline_last_stage():
        ...
        if from_next:
            ...
            # wppline: recv backward from prev rank
...
def send_forward(output_tensor: torch.Tensor,
                 batch_p2p_comm: bool = True,
                 timers: Callable = None,
                 to_next: bool = True) -> None:
    """Send tensor to next rank in pipeline (forward send).

    See _communicate for argument details.
    
    to_next: in wpp, some micro batch may send forward to prev rank
    """
...
def send_backward(input_tensor_grad: torch.Tensor,
                  batch_p2p_comm: bool = True,
                  timers: Callable = None,
                  to_prev: bool = True) -> None:
    """Send tensor to previous rank in pipeline (backward send).

    See _communicate for argument details.
    
    to_prev: in wpp, some micro batch may send backward to next rank
    """
...
def send_forward_recv_backward(output_tensor: torch.Tensor,
                               ...
                               batch_p2p_comm: bool = True,
                               timers: Callable = None,
                               with_next: bool = True) -> torch.Tensor:
    """Batched send and recv with next rank in pipeline.

    See _communicate for argument details.
    
    with_next: in wpp, some micro batch may send and recv with prev rank
    """
    if core.parallel_state.is_pipeline_last_stage():
        output_tensor_grad = None
    else:
...
def send_backward_recv_forward(input_tensor_grad: torch.Tensor,
                               ...
                               batch_p2p_comm: bool = True,
                               timers: Callable = None,
                               with_prev: bool = True) -> torch.Tensor:
    """Batched send and recv with previous rank in pipeline.

    See _communicate for argument details.
    
    with_prev: in wpp, some micro batch may send and recv with next rank
    """
    if core.parallel_state.is_pipeline_first_stage():
        input_tensor = None
    else:
