...
_WPP_MODEL_PARALLEL_WORLD_SIZE = None
...
def initialize_model_parallel(
    ...
    wpp_model_parallel_size: Optional[int] = None,
    ...
    if wpp_model_parallel_size is not None:
        global _WPP_MODEL_PARALLEL_WORLD_SIZE
        _WPP_MODEL_PARALLEL_WORLD_SIZE = wpp_model_parallel_size
    ...
    global _PIPELINE_IS_1F1B_ENABLE
    _PIPELINE_IS_1F1B_ENABLE = use_1f1b
...
def set_wpp_model_parallel_world_size(world_size):
    """Set the pipeline model parallel size"""
    global _WPP_MODEL_PARALLEL_WORLD_SIZE
    _WPP_MODEL_PARALLEL_WORLD_SIZE= world_size
    
def set_pipeline_is_1f1b_enable(enable):
    global _PIPELINE_IS_1F1B_ENABLE
    _PIPELINE_IS_1F1B_ENABLE = enable
...
def get_pipeline_is_1f1b_enable():
    """Return whether enable 1f1b."""
    global _PIPELINE_IS_1F1B_ENABLE
    if _PIPELINE_IS_1F1B_ENABLE is not None:
        return _PIPELINE_IS_1F1B_ENABLE
...
def is_pipeline_last_stage(ignore_virtual=False, wpp=False):
    """Return True if in the last pipeline model-parallel stage, False otherwise."""
    if ignore_virtual and wpp:
        return get_pipeline_model_parallel_rank() == 0
    if not ignore_virtual:
        virtual_pipeline_model_parallel_world_size = \
            get_virtual_pipeline_model_parallel_world_size()
        wpp_model_parallel_world_size = \
            get_wpp_model_parallel_world_size()
        if wpp_model_parallel_world_size is not None:
...
def get_wpp_model_parallel_world_size():
    """Return the wpp model-parallel world size"""
    global _WPP_MODEL_PARALLEL_WORLD_SIZE
    return _WPP_MODEL_PARALLEL_WORLD_SIZE
