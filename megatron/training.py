...
def pretrain(train_valid_test_dataset_provider,
    ...
    directory = os.path.dirname(args.save+'/log')
    if directory != '':
        os.makedirs(directory, exist_ok=True)
    if args.save:
        if torch.distributed.get_rank() == 0:        
            ...
            with open(args.save+'/params.json', 'w') as json_file:
                json.dump(params, json_file, indent=4)
            # log = {'throughput':[]}
            # with open(args.save+'/log', 'w') as json_file:
            #     json.dump(log, json_file, indent=4)
        with open(args.save+'/timerlog%d.json'%torch.distributed.get_rank(), 'w') as json_file:
            json.dump({}, json_file, indent=4)
        with open(args.save+'/memorylog%d.json'%torch.distributed.get_rank(), 'w') as json_file:
    ...
    with open(args.save+'/optlog%d.txt'%rank, 'w') as file:
        # write model
        file.write(str(model)+'\n\n')
        file.write('-----\n')
        file.write(str(optimizer.models)+'\n')
    ...
    timers.record(args.save)
    if torch.distributed.get_rank() == 0:
        timers.record_log_level(args.save)
...
def setup_model_and_optimizer(model_provider_func,
    ...
    print(torch.distributed.get_rank(), 'get model')
...
def train_step(forward_step_func, data_iterator,
    ...
    losses_reduced, dependencies = forward_backward_func(
    ...
    if get_dependency:
        ...
        with open(f"{args.save}/dependency{pipeline_rank}.json", 'w') as json_file:
    ...
    flag_wpp = (mpu.get_wpp_model_parallel_world_size() != None)
    if mpu.is_pipeline_last_stage(ignore_virtual=True, wpp=flag_wpp):
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        get_dependency = (iteration == 0) and (args.num_waves_per_pipeline is not None)
        ...
        with open(record_path, 'r') as file:
            data = json.load(file)
        data['max-memory'] = torch.cuda.max_memory_allocated()
        with open(record_path, 'w') as json_file:
            json.dump(data, json_file, indent=4)
...
def evaluate(forward_step_func,
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            forward_backward_func = get_forward_backward_func()
            loss_dicts, _ = forward_backward_func(
                forward_step_func=forward_step_func,
                data_iterator=data_iterator,
                model=model,
                num_microbatches=get_num_microbatches(),
        ...
        if process_non_loss_data_func is not None and is_last_rank():
            collected_non_loss_data, _ = forward_backward_func(
