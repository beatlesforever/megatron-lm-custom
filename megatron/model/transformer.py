...
def _get_num_layers(args, model_type, is_decoder=False):
    ...
    if model_type == ModelType.retro_encoder:
        ...
        if is_encoder_and_decoder_model:
            ...
            #     num_layers -= 1
            #     print('----', torch.distributed.get_rank(), num_layers)
    else:
        if not is_decoder:
    ...
    def __init__(self, init_method, output_layer_init_method,
        ...
        if args.num_waves_per_pipeline is not None:
            ...
            if args.model_type == ModelType.encoder_and_decoder and \
                ...
                # 不对称切分
                # if torch.distributed.get_rank() > 0:
                #     offset = mpu.get_pipeline_model_parallel_rank() * self.num_layers + 1
                # if torch.distributed.get_rank() == args.world_size - 1:
                #     offset = mpu.get_pipeline_model_parallel_rank() * (self.num_layers + 1) + 1
                    # print('last stage num layers -- ')
                    # print(self.num_layers, 'offset', offset)
